...
tab1, tab2, tab3 = st.tabs(['Stock Performance','Stocks Financials','Sector Performance']) 
...
with tab1:
    ...
    intervals = [ "5m", "15m","60m", "1d", "5d", "1mo", "3mo", "6mo", "1y"]
    ...
    weekends = pd.date_range(start = start, end= end, freq='W-SAT').tolist()+ pd.date_range(start = start, end= end, freq='W-SUN').tolist()
    weekends_df = pd.DataFrame({'holiday':'weekend','ds':pd.to_datetime(weekends),'lower_window': 0,'upper_window':1 })

    # Initialize and fit the model
    model = Prophet(holidays=weekends_df)
    ...
    forecast = forecast.tail(forecast_period)
    df_prophet_predict = df_prophet.tail(100)
    # Plot forecast
    fig_forecast = go.Figure()
    fig_forecast.add_trace(go.Scatter(x=forecast['ds'], y=forecast['yhat'], mode='lines', name='Forecast'))
    fig_forecast.add_trace(go.Scatter(x=df_prophet_predict['ds'], y=df_prophet_predict['y'], mode='lines', name='Historical Data'))
...
with tab2:
    custom_css = """<style>.wide-table {width: 100%;border-collapse: collapse;}wide-table th, .wide-table td {border: 1px solid black;padding: 8px;text-align: left;}wide-table th {background-color: #f2f2f2;}</style>"""
    ticker = yf.Ticker(ticker)
    
    if st.button('Balance Sheet'):
            bal_sheet = ticker.balance_sheet
            bal_sheet.columns = bal_sheet.columns.strftime('%Y-%m-%d')
            bal_sheet = bal_sheet.to_html(classes='wide-table')
            st.markdown(custom_css +bal_sheet,unsafe_allow_html= True)
    
    if st.button('QTR Financials'):
            qtr_reports = ticker.quarterly_balance_sheet
            qtr_reports.columns = qtr_reports.columns.strftime('%Y-%m-%d')
            st.table(qtr_reports)
